// src/components/HarmonicIntervalExercise.tsx
'use client';
...
interface HarmonicIntervalExerciseProps {
  difficulty: 'beginner' | 'intermediate' | 'advanced';
  onComplete?: (result: {
    correct: boolean;
    userAnswer: string;
...
const HarmonicIntervalExercise: React.FC<HarmonicIntervalExerciseProps> = ({
  difficulty,
  onComplete
}) => {
  ...
  // ✅ FUNÇÃO DE TOCAR INTERVALO HARMÔNICO - DUAS NOTAS SIMULTANEAMENTE COM DURAÇÃO
  const playInterval = useCallback(async () => {
    ...
    try {
      ...
      console.log(`🎵 Tocando intervalo harmônico: ${lowerName} (${lowerFreq.toFixed(1)}Hz) + ${upperName} (${upperFreq.toFixed(1)}Hz)`);
      ...
      if (typeof playNote === 'function' && typeof stopNote === 'function') {
        // ✅ TOCAR AMBAS AS NOTAS SIMULTANEAMENTE
        await Promise.all([
          playNote(lowerName, lowerFreq),
          playNote(upperName, upperFreq)
        ]);

        // Deixar tocar por uma duração similar aos melódicos (2 segundos total)
      ...
      console.error('❌ Erro ao tocar intervalo harmônico:', err);
      setIsPlaying(false);
    }
  ...
  const checkAnswer = useCallback(async () => {
    ...
    try {
      ...
      const sessionData: SessionResult = {
        exerciseType: 'harmonic-intervals', // ✅ MUDADO DE melodic-intervals
        difficulty,
        totalQuestions: 1,
  ...
  const getIntervalStats = useCallback(() => {
    ...
    return userProgress.exerciseStats.find(stat => stat.exerciseType === 'harmonic-intervals');
  }, [userProgress]);
            ...
            <div className="text-center">
              ...
              <div className="flex flex-col items-center gap-2">
                <span className={`px-2 py-1 rounded-full text-white text-xs sm:text-sm font-medium ${
                  difficulty === 'beginner' ? 'bg-purple-500' :
                  difficulty === 'intermediate' ? 'bg-purple-600' :
                  'bg-purple-700'
                }`}>
                  {difficulty === 'beginner' ? 'Iniciante' : difficulty === 'intermediate' ? 'Intermediário' : 'Avançado'}
                </span>
                <span className="text-gray-600 text-xs text-center max-w-xs">
                  Ouça duas notas tocadas simultaneamente
                  <br />
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
              <div className="text-center">
                ...
                <p className="mt-4 text-gray-600 text-sm">
                  Clique para ouvir o intervalo harmônico (duas notas simultaneamente)
                </p>
                ...
                {currentInterval && (
                  <div className="mt-3 text-xs text-gray-500">
                    ...
                    <span className="ml-2">• 🎹 Notas simultâneas</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6">
              <h3 className="text-lg sm:text-xl font-bold mb-4 sm:mb-6 text-center text-gray-800">
                Qual intervalo harmônico você ouviu?
              </h3>
                  ...
                  <button
                    ...
                    className={`p-4 rounded-lg text-left transition-all ${
                      showResult
                        ...
                        ? 'bg-purple-100 border-2 border-purple-500 text-purple-900 shadow-md'
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex justify-between items-center py-2">
                  ...
                  <span className="font-bold text-purple-600">{intervalStats?.totalXpEarned || 0}</span>
                </div>
              </div>
            </div>
            ...
            {showResult && (
              <div className="space-y-4">
                ...
                <button
                  ...
                  className="w-full bg-purple-600 text-white py-3 sm:py-4 px-4 sm:px-6 rounded-xl font-bold text-base sm:text-lg hover:bg-purple-700 transition-all shadow-lg hover:shadow-xl transform hover:scale-[1.02]"
                >
                  <div className="flex items-center justify-center gap-2">
...
export default HarmonicIntervalExercise;