...
const llamaProvider_1 = require("./providers/llamaProvider");
const geminiProvider_1 = require("./providers/geminiProvider");
const huggingfaceProvider_1 = require("./providers/huggingfaceProvider");
const groqProvider_1 = require("./providers/groqProvider"); // ADICIONADO
// Implementação do serviço LLM
class LLMService {
    // Inicialização com provider configurável
    static initialize(providerType = 'groq') {
        switch (providerType.toLowerCase()) {
            case 'groq': // ADICIONADO
                this.provider = new groqProvider_1.GroqProvider();
                console.log('🚀 LLM Service initialized with Groq (FREE & FAST)');
                break;
            case 'huggingface':
                this.provider = new huggingfaceProvider_1.HuggingFaceProvider();
                console.log('🤗 LLM Service initialized with Hugging Face (FREE)');
                break;
            case 'deepseek':
                this.provider = new deepseek_1.DeepseekProvider();
                break;
            case 'llama':
                this.provider = new llamaProvider_1.LlamaProvider();
                break;
            case 'gemini':
                this.provider = new geminiProvider_1.GeminiProvider();
                break;
            default:
                this.provider = new groqProvider_1.GroqProvider(); // MUDADO: default agora é Groq
                console.log('🚀 LLM Service initialized with Groq (DEFAULT FREE & FAST)');
        }
    }
    // Gerar sequência MIDI para exercício
    static async generateMIDISequence(exerciseType, difficulty, options = {}) {
    ...
    `;
        ...
        try {
            ...
            const response = await this.provider.generateResponse(systemPrompt, exercisePrompt, {
                outputFormat: 'json',
                temperature: 0.7,
                maxTokens: 1500
            });
            ...
            let midiSequence;
            try {
                midiSequence = JSON.parse(response);
            }
            catch (parseError) {
                ...
                if (jsonMatch) {
                    midiSequence = JSON.parse(jsonMatch[0]);
                }
                else {
    ...
    static getMIDIGenerationPrompt(exerciseType, difficulty, options) {
        switch (exerciseType) {
            case 'interval':
                return `
    ...
    static validateAndCompleteMIDISequence(sequence, exerciseType, difficulty) {
    ...
    static calculateRhythmValues(sequence) {
    ...
    static getDefaultEvents(exerciseType) {
        switch (exerciseType) {
            case 'interval':
                return [
    ...
    static generateFallbackMIDISequence(exerciseType, difficulty) {
        return {
