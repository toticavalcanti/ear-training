...
exports.getUserStats = exports.getExerciseHistory = exports.checkAnswer = exports.getExercise = void 0;
...
const getExercise = async (req, res) => {
    try {
        ...
        if (!type || !['interval', 'progression', 'melodic', 'rhythmic'].includes(type)) {
        ...
        const shouldUseLLM = (useLLM === 'true' || useLLM === '1') && // Explicitly requested
            isPremiumUser; // Premium users only
        let exercise;
        if (shouldUseLLM) {
            // Generate exercise using LLM
            const newExercise = await (0, exerciseService_1.generateLLMExercise)(type, difficulty || 'beginner', {}, // Options can be expanded later
            isPremiumUser);
            exercise = await Exercise_1.default.create(newExercise);
        }
        else {
            // Try to find an existing exercise of the requested type
            exercise = await Exercise_1.default.findOne({
                type,
            ...
            if (!exercise) {
                ...
                switch (type) {
                    case 'interval':
                        newExercise = (0, exerciseService_1.generateIntervalExercise)(difficulty || 'beginner', isPremiumUser);
                        break;
                    case 'progression':
                        newExercise = (0, exerciseService_1.generateProgressionExercise)(difficulty || 'beginner', isPremiumUser);
                        break;
                    case 'melodic':
                        newExercise = (0, exerciseService_1.generateMelodicExercise)(difficulty || 'beginner', isPremiumUser);
                        break;
                    case 'rhythmic':
                        newExercise = (0, exerciseService_1.generateRhythmicExercise)(difficulty || 'beginner', isPremiumUser);
                        break;
                    default:
...
const checkAnswer = async (req, res) => {
    try {
        ...
        await ExerciseHistory_1.default.create({
            userId: user._id,
...
const getExerciseHistory = async (req, res) => {
    try {
        ...
        const history = await ExerciseHistory_1.default.find(query)
            .sort({ date: -1 })
            .skip(skip)
            .limit(Number(limit));
        const total = await ExerciseHistory_1.default.countDocuments(query);
        res.json({
            history,
            pagination: {
                total,
...
const getUserStats = async (req, res) => {
    try {
        ...
        const totalExercises = await ExerciseHistory_1.default.countDocuments({ userId: user._id });
        ...
        const dailyStats = await ExerciseHistory_1.default.aggregate([
            {
                $match: {
                    userId: user._id,
...
async function getStatsByType(userId, type) {
    const total = await ExerciseHistory_1.default.countDocuments({
        userId,
        exerciseType: type
    });
    const correct = await ExerciseHistory_1.default.countDocuments({
        userId,
        exerciseType: type,
        isCorrect: true
    });
    // Obter estatísticas por dificuldade
    const byDifficulty = await ExerciseHistory_1.default.aggregate([
        {
            $match: {
                userId,
                exerciseType: type
